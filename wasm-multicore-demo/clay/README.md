...
`Clay_RenderCommand.textContent` will be populated with a `Clay_String` _array_ of the original string passed in (i.e. wrapping doesn't reallocate, it just returns a `Clay_String` pointing to the start of the new line with a `length`)
