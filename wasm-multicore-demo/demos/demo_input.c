...
void app_update_and_render(AppMemory *memory) {
  ...
  if (should_log_state && is_main_thread()) {
    ...
    LOG_INFO(
        "Mouse pos: (%, %) delta: (%, %) scroll: (%, %)",
        FMT_FLOAT(g_input.mouse_pos[0]), FMT_FLOAT(g_input.mouse_pos[1]),
        FMT_FLOAT(g_input.mouse_delta[0]), FMT_FLOAT(g_input.mouse_delta[1]),
        FMT_FLOAT(g_input.scroll_delta[0]), FMT_FLOAT(g_input.scroll_delta[1]));
  ...
  for (u32 i = 0; i < KEY_MAX; i++) {
    if (g_input.buttons[i].pressed_this_frame) {
      LOG_INFO("[Thread %] % PRESSED", FMT_UINT(thread_idx),
               FMT_STR(input_button_names[i]));
    }
    if (g_input.buttons[i].released_this_frame) {
      LOG_INFO("[Thread %] % RELEASED", FMT_UINT(thread_idx),
               FMT_STR(input_button_names[i]));
    }
  }
  ...
  if (is_main_thread()) {
    for (u32 i = 0; i < g_input.chars_len; i++) {
      LOG_INFO("CHAR: '%' (codepoint: %)", FMT_CHAR((char)g_input.chars[i]),
               FMT_UINT(g_input.chars[i]));
    }
  }

  if (is_main_thread()) {
    if (g_input.buttons[KEY_F11].pressed_this_frame) {
      os_set_fullscreen(!os_is_fullscreen());
    }
  }
