...
u32 js_gpu_make_shader(const char *vs_code, u32 vs_len, const char *fs_code, u32 fs_len);

WASM_IMPORT(js_gpu_destroy_shader)
void js_gpu_destroy_shader(u32 handle_idx);

WASM_IMPORT(js_gpu_make_pipeline)
u32 js_gpu_make_pipeline(u32 shader_idx, void *vertex_layout, u32 primitive, u32 depth_test, u32 depth_write);
...
GpuShader gpu_make_shader(GpuShaderDesc *desc) {
    ...
    u32 vs_len = 0;
    u32 fs_len = 0;
    for (const char *p = desc->vs_code; *p; p++) vs_len++;
    for (const char *p = desc->fs_code; *p; p++) fs_len++;
