...
internal void win32_add_key_event(AppInputEvents *events,
                                  App_InputButtonType key, b32 is_down) {
  if (key == KEY_MAX || events->len >= GAME_INPUT_EVENTS_MAX_COUNT)
    return;
...
internal LRESULT CALLBACK win32_window_callback(HWND hwnd, UINT msg,
...
internal HWND win32_create_window(HINSTANCE instance, i32 width, i32 height) {
  ...
  HWND hwnd = CreateWindowExA(
      0, wc.lpszClassName, "Wasm Multicore Demo", WS_OVERLAPPEDWINDOW,
...
b32 os_is_mouse_locked(void) { return g_mouse_locked; }
...
void os_set_fullscreen(b32 fullscreen) {
  ...
  if (fullscreen) {
    ...
    if (GetMonitorInfoA(MonitorFromWindow(g_hwnd, MONITOR_DEFAULTTOPRIMARY),
                        &mi)) {
      SetWindowLongA(g_hwnd, GWL_STYLE,
                     g_windowed_style & ~WS_OVERLAPPEDWINDOW);
...
b32 os_is_fullscreen(void) { return g_fullscreen; }
...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE prev_instance, LPSTR cmd_line,
  ...
  for (u8 i = 1; i < g_app_ctx.num_threads; i++) {
    ...
    sb_append_format(&sb, "Thread %", FMT_UINT(i));
