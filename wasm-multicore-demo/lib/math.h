...
force_inline b32 aabb_in_frustum(Frustum *frustum, vec3 min, vec3 max) {
  for (u32 i = 0; i < 6; i++) {
    Plane plane = frustum->planes[i];

    vec3 positive_vertex = {
      plane.normal[0] > 0.0f ? max[0] : min[0],
      plane.normal[1] > 0.0f ? max[1] : min[1],
      plane.normal[2] > 0.0f ? max[2] : min[2]
    };

    f32 distance = glm_vec3_dot(plane.normal, positive_vertex) + plane.distance;
...
force_inline b32 aabb2d_in_frustum(Frustum *frustum, vec3 center, f32 half_size_x, f32 half_size_z) {
  vec3 min = {center[0] - half_size_x, center[1], center[2] - half_size_z};
  vec3 max = {center[0] + half_size_x, center[1], center[2] + half_size_z};
  return aabb_in_frustum(frustum, min, max);
}
