...
String str_from_cstr_with_len_alloc(const char *cstr, u32 len,
                                    Allocator *allocator) {
  char *s = ALLOC_ARRAY(allocator, char, len + 1);
  memcpy(s, cstr, len);
  s[len] = '\0';
...
b32 str_ends_with(const char *str, const char *suffix) {
  if (!str || !suffix) {
    return false;
  }
  ...
  return str_equal(str + slen - suffix_len, suffix);
}

const char *str_find(const char *str, const char *pattern) {
  if (!str || !pattern) {
    return NULL;
  }
...
i32 str_indexof(const char *str, const char *pattern) {
  if (!str || !pattern) {
    return -1;
  }
