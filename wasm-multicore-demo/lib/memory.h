...
/*
    ArenaAllocator - bump allocator with optional reserve/commit support

    .buffer: pointer to memory buffer (base address)
    .reserved: total reserved address space (was 'capacity')
    .committed: physical memory backed (for virtual memory arenas)
    .offset: current allocation offset (grows with each allocation)
    .commit_size: granularity for new commits (e.g., 64KB), 0 = no commit needed
    .owns_memory: whether arena should release memory on destroy
*/
typedef struct {
  uint8 *buffer;
  size_t reserved;
...
/* create arena allocator from existing buffer (fully committed, no virtual memory) */
HZ_ENGINE_API ArenaAllocator arena_from_buffer(uint8 *buffer, size_t capacity);

/* create arena with virtual memory (reserve large, commit on demand) */
HZ_ENGINE_API ArenaAllocator arena_create(size_t reserve_size, size_t commit_size);

/* create arena from existing reserved buffer with commit-on-demand support */
HZ_ENGINE_API ArenaAllocator arena_from_reserved_buffer(uint8 *buffer,
                                                         size_t reserved_size,
                                                         size_t committed_size,
                                                         size_t commit_size);

/* release arena memory (only if arena owns the memory) */
HZ_ENGINE_API void arena_release(ArenaAllocator *arena);
