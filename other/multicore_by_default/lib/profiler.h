/*
    ...
    --- Thread-safe CPU profiler using TSC (time stamp counter)

    --- measures inclusive and exclusive time for profiled scopes

    --- use PROFILE_BEGIN/PROFILE_END macros to mark scopes

    USAGE
        void my_function() {
            PROFILE_BEGIN("my_function");
...
extern thread_local ProfileAnchor g_profiler_anchors[PROFILER_MAX_ANCHORS];
extern thread_local u32 g_profiler_parent;
extern thread_local int g_thread_registered;
...
extern thread_local ProfileBlock* g_profile_stack[PROFILER_MAX_STACK_DEPTH];
extern thread_local u32 g_profile_stack_depth;
...
#define PROFILE_ASSERT_END_OF_COMPILATION_UNIT static_assert(__COUNTER__ < PROFILER_MAX_ANCHORS, "Number of profile points exceeds size of profiler anchors array")
...
static inline void profiler_end_and_print_session(Allocator* allocator) {UNUSED(allocator);}
