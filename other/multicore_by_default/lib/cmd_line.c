...
b32 cmdline_parse(CmdLineParser *parser, i32 argc, char *argv[]) {
  ...
  for (i32 i = 1; i < argc; i++) {
    ...
    if (arg_len > 2 && arg_cstr[0] == '-' && arg_cstr[1] == '-') {
      ...
      if (!found_flag) {
        printf("Error: Unknown flag/option: %s\n", arg_cstr);
        return false;
      }
    } else {
      if (!parsing_commands) {
        printf("Error: Commands must come before flags. Found '%s' after flags.\n", arg_cstr);
        return false;
      }
      ...
      if (!valid_command && parser->registered_commands.len > 0) {
        printf("Error: Unknown command '%s'. Valid commands are:\n", arg_cstr);
        for (u32 j = 0; j < parser->registered_commands.len; j++) {
          printf("  %.*s\n", (int)parser->registered_commands.items[j].len,
                 parser->registered_commands.items[j].value);
        }
        return false;
      }
