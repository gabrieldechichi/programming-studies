...
WASM_IMPORT(_os_is_mobile) b32 _os_is_mobile();
...
WASM_IMPORT(_os_start_read_file) i32 _os_start_read_file(const char *file_name, int length);
WASM_IMPORT(_os_check_read_file) i32 _os_check_read_file(i32 op_id);
WASM_IMPORT(_os_get_file_size) i32 _os_get_file_size(i32 op_id);
WASM_IMPORT(_os_get_file_data) void _os_get_file_data(i32 op_id, u8 *buffer_ptr, u32 buffer_len);
...
WASM_IMPORT(_platform_audio_write_samples) void _platform_audio_write_samples(f32 *samples, i32 sample_count);
WASM_IMPORT(_platform_audio_get_sample_rate) i32 _platform_audio_get_sample_rate();
WASM_IMPORT(_platform_audio_get_samples_needed) u32 _platform_audio_get_samples_needed();
WASM_IMPORT(_platform_audio_update) void _platform_audio_update();
WASM_IMPORT(_platform_audio_shutdown) void _platform_audio_shutdown();
...
WASM_IMPORT(_os_lock_mouse) void _os_lock_mouse(b32 lock);
WASM_IMPORT(_os_is_mouse_locked) b32 _os_is_mouse_locked();

void os_lock_mouse(b32 lock) { _os_lock_mouse(lock); }

b32 os_is_mouse_locked(void) { return _os_is_mouse_locked(); }

WASM_IMPORT(_os_get_compressed_texture_format) i32 _os_get_compressed_texture_format();
