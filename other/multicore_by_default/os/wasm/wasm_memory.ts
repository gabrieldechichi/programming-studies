export class WasmMemoryInterface {
  ...
  loadF32Array(addr: number, len: number): Float32Array {
    ...
    return new Float32Array(this.memory.buffer, addr >>> 0, len);
  }
  ...
  loadF64Array(addr: number, len: number): Float64Array {
    ...
    return new Float64Array(this.memory.buffer, addr >>> 0, len);
  }
  ...
  loadU32Array(addr: number, len: number): Uint32Array {
    ...
    return new Uint32Array(this.memory.buffer, addr >>> 0, len);
  }
  ...
  loadI32Array(addr: number, len: number): Int32Array {
    ...
    return new Int32Array(this.memory.buffer, addr >>> 0, len);
  }
  ...
  loadI16Array(addr: number, len: number): Int16Array {
    ...
    return new Int16Array(this.memory.buffer, addr >>> 0, len);
  }

  loadU16Array(addr: number, len: number): Uint16Array {
    if (!this.memory) {
      throw new Error("Memory not set");
    }
    return new Uint16Array(this.memory.buffer, addr >>> 0, len);
  }
  ...
  loadU8Array(addr: number, len: number): Uint8Array {
    ...
    return new Uint8Array(this.memory.buffer, addr >>> 0, len);
  }

  loadU8(addr: number): number {
    return this.mem.getUint8(addr >>> 0);
  }
  loadI8(addr: number): number {
    return this.mem.getInt8(addr >>> 0);
  }
  loadU16(addr: number): number {
    return this.mem.getUint16(addr >>> 0, true);
  }
  loadI16(addr: number): number {
    return this.mem.getInt16(addr >>> 0, true);
  }
  ...
  loadF32(addr: number): number {
    return this.mem.getFloat32(addr >>> 0, true);
  }
  loadF64(addr: number): number {
    return this.mem.getFloat64(addr >>> 0, true);
  }
  ...
  storeU8Array(addr: number, values: Uint8Array): void {
    new Uint8Array(this.mem.buffer, addr >>> 0, values.length).set(values);
  }

  storeU8(addr: number, value: number): void {
    this.mem.setUint8(addr >>> 0, value);
  }
  storeI8(addr: number, value: number): void {
    this.mem.setInt8(addr >>> 0, value);
  }
  storeU16(addr: number, value: number): void {
    this.mem.setUint16(addr >>> 0, value, true);
  }
  storeI16(addr: number, value: number): void {
    this.mem.setInt16(addr >>> 0, value, true);
  }
  storeU32(addr: number, value: number): void {
    this.mem.setUint32(addr >>> 0, value, true);
  }
  storeI32(addr: number, value: number): void {
    this.mem.setInt32(addr >>> 0, value, true);
  }
  ...
  storeF32(addr: number, value: number): void {
    this.mem.setFloat32(addr >>> 0, value, true);
  }
  storeF64(addr: number, value: number): void {
    this.mem.setFloat64(addr >>> 0, value, true);
  }
  ...
  storeString(addr: number, value: string): number {
    ...
    const dst = new Uint8Array(this.memory.buffer, addr >>> 0, src.length);
    dst.set(src);
    return src.length;
  }
}
