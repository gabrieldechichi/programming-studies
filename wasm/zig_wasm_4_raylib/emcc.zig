...
pub fn linkWithEmscripten(
    ...
    const mkdir_command = b.addSystemCommand(&[_][]const u8{ "Powershell.exe", "-Command", "New-Item", " -ItemType Directory", "-Force", "-Path", emccOutputDir });
    ...
    for (itemsToLink) |item| {
        ...
        emcc_command.link_step.dependOn(&item.step);
    }
    // This puts the file in zig-out/htmlout/index.html.
    emcc_command.link_step.dependOn(&mkdir_command.link_step);
