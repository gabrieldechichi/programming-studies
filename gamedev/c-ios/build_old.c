...
#define MACOS_C_COMPILE_FLAGS                                                  \
  "-std=c11 -Isrc -DMACOS=1 " MAIN_STRICT_FLAGS
...
static int build_macos(const char *build_type) {
  ...
  snprintf(cmd, sizeof(cmd), "%s %s %s %s %s -c %s -o %s",
           ...
           "-I/opt/homebrew/include",  // FFmpeg headers
  ...
  snprintf(cmd, sizeof(cmd), "%s %s %s %s %s -o %s %s %s",
           CC,
           MACOS_VIDEO_OBJ, MACOS_GPU_OBJ, MACOS_PROFILER_OBJ,
           "",  // Additional object files if needed
           MACOS_APP_TARGET,
