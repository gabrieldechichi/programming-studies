...
AudioState audio_init(GameContext *ctx) {
  AudioState state = {0};
  state.output_sample_rate = platform_audio_get_sample_rate();
  state.output_channels = 2;
  // todo: forcing fixed frame rate here, use target frame rate from game
  state.max_samples_per_frame = state.output_sample_rate / 20.0f;
  state.clips = slice_new_ALLOC(&ctx->allocator, AudioClip, 8);
  state.streaming_clips =
      slice_new_ALLOC(&ctx->allocator, StreamingAudioClip, 16);
...
void audio_update(AudioState *state, GameContext *ctx, f32 dt) {
  ...
  i32 buffer_size = state->max_samples_per_frame * state->output_channels;
  ...
  for (i32 clip_idx = state->clips.len - 1; clip_idx >= 0; clip_idx--) {
    ...
    if (!clip->is_playing || !clip->wav_file || !clip->wav_file->is_loaded) {
      slice_remove_swap(state->clips, clip_idx);
      continue;
    }
    did_write = true;
  ...
  for (i32 clip_idx = state->streaming_clips.len - 1; clip_idx >= 0;
    ...
    if (!clip->is_playing) {
      slice_remove_swap(state->streaming_clips, clip_idx);
      continue;
    }
...
void audio_play_clip(AudioState *state, AudioClip clip) {
  if (clip.wav_file && clip.wav_file->is_loaded) {
    ...
    i32 target_rate = platform_audio_get_sample_rate();
    clip.sample_rate_ratio =
        (f32)clip.wav_file->format.sample_rate / (f32)target_rate;

    // Add to clips slice
    slice_append(state->clips, clip);
  }
}
...
b32 streaming_clip_get_sample(StreamingAudioClip *clip, f32 position, f32 *left,
  ...
  // Calculate byte offset for 16-bit samples
  u32 bytes_per_sample = sizeof(i16) * clip->channels;
  u32 available_data = streaming_buffer_available_data_len(buffer);
  u32 available_samples = available_data / bytes_per_sample;

  // Check if we have enough data for current sample
  if (sample_index >= available_samples) {
    *left = *right = 0.0f;
    return false;
  }

  // Get samples from circular buffer starting from read_pos
  u32 byte_offset = sample_index * bytes_per_sample;
  u32 pos1 = (buffer->read_pos + byte_offset) % buffer->capacity;
  u32 pos2 =
      (buffer->read_pos + byte_offset + bytes_per_sample) % buffer->capacity;

  if (clip->channels == 1) {
    // Mono - get sample and duplicate to stereo
    i16 sample1 = *(i16 *)(buffer->buffer + pos1);
    i16 sample2 = sample1;

    // Try to get next sample for interpolation if available
    if (sample_index + 1 < available_samples) {
      sample2 = *(i16 *)(buffer->buffer + pos2);
    }
    ...
    *left = *right = interpolated;
    ...
    // Stereo
    i16 left1 = *(i16 *)(buffer->buffer + pos1);
    i16 right1 = *(i16 *)(buffer->buffer + pos1 + 2);

    i16 left2 = left1;
    i16 right2 = right1;

    // Try to get next samples for interpolation if available
    if (sample_index + 1 < available_samples) {
      left2 = *(i16 *)(buffer->buffer + pos2);
      right2 = *(i16 *)(buffer->buffer + pos2 + 2);
    }
...
StreamingAudioClip *audio_play_streaming_clip(AudioState *state,
  ...
  slice_append(state->streaming_clips, clip);
