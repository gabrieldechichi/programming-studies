...
HttpRequest http_get_async(const char *url, Allocator *arena) {
  ...
  // request.response.error_message = NULL;

  LOG_INFO("Sending http request %", FMT_STR(url));
  request.platform_op = platform_start_http_request(
      HTTP_METHOD_GET, url, strlen(url), "", 0, // No headers
      "", 0                                     // No body
  );
...
HttpRequest http_post_async(const char *url, const char *headers,
  ...
  request.platform_op = platform_start_http_request(
      HTTP_METHOD_POST, url, strlen(url), headers ? headers : "",
      headers ? strlen(headers) : 0, body ? body : "", body ? strlen(body) : 0);
...
bool32 http_request_is_complete(HttpRequest *request) {
  if (!request) {
    return true; // Invalid request is "complete"
  }
  ...
  HttpOpState state = platform_check_http_request(request->platform_op);

  if (state == HTTP_OP_COMPLETED || state == HTTP_OP_ERROR) {
    // Process the response
    if (state == HTTP_OP_COMPLETED) {
      int32 status_code, headers_len, body_len;
      int32 result = platform_get_http_response_info(
          request->platform_op, &status_code, &headers_len, &body_len);

      if (result == 0) {
        request->response.status_code = status_code;
        request->response.success = (status_code >= 200 && status_code < 300);

        // Allocate and get response body
        if (body_len > 0) {
          request->response.body =
              ALLOC_ARRAY(request->arena, char, body_len + 1);
          if (request->response.body) {
            int32 body_result = platform_get_http_body(
                request->platform_op, request->response.body, body_len);

            if (body_result == 0) {
              request->response.body_len = body_len;
              request->response.body[body_len] = '\0'; // Null terminate
            } else {
...
// Streaming HTTP implementations
HttpStreamRequest http_stream_get_async(const char *url, Allocator *arena) {
  ...
  request.platform_op = platform_start_http_stream(
      HTTP_METHOD_GET, url, strlen(url), "", 0, // No headers
      "", 0                                     // No body
  );
...
HttpStreamRequest http_stream_post_async(const char *url, const char *headers,
  ...
  request.platform_op = platform_start_http_stream(
      HTTP_METHOD_POST, url, strlen(url), headers,
      headers ? strlen(headers) : 0, body, body ? strlen(body) : 0);
...
HttpStreamRequest http_stream_post_binary_async(const char *url,
  ...
  LOG_INFO("Starting http stream POST binary request % (% bytes)", FMT_STR(url),
           FMT_UINT(body_len));
  request.platform_op =
      platform_start_http_stream(HTTP_METHOD_POST, url, strlen(url), headers,
                                 headers ? strlen(headers) : 0, body, body_len);
...
bool32 http_stream_is_ready(HttpStreamRequest *request) {
  ...
  HttpStreamState state = platform_check_http_stream(request->platform_op);

  if (state == HTTP_STREAM_READY || state == HTTP_STREAM_HAS_CHUNK) {
    if (!request->stream_ready) {
      // Get initial response info (status code, etc.)
      int32 status_code;
      int32 result =
          platform_get_http_stream_info(request->platform_op, &status_code);
      if (result == 0) {
...
HttpStreamChunk http_stream_get_chunk(HttpStreamRequest *request) {
  ...
  int32 chunk_size = platform_get_http_stream_chunk_size(request->platform_op);
  if (chunk_size <= 0) {
    return chunk;
  }
  ...
  bool32 is_final;
  int32 result = platform_get_http_stream_chunk(
      request->platform_op, chunk.chunk_data, chunk_size, &is_final);
