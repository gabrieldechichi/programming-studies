...
void f_debug_mem_thread_safe_init(int (*lock)(void *mutex),
                                  int (*unlock)(void *mutex), void *mutex) {
  f_alloc_mutex = mutex;
  f_alloc_mutex_lock = lock;
  f_alloc_mutex_unlock = unlock;
}

void f_debug_mem_stack_pointer_set(void *lowest_stack_pinter,
      ...
      if (f_alloc_lines[i].allocs[j].active) {
        ...
        if (k < FORGE_MEMORY_OVER_ALLOC - FORGE_MEMORY_PRE_PADDIG) {
          if (f_alloc_lines[i].allocs[j].comment == NULL)
            printf("FORGE Mem debugger error: Memory Overrun of allocation "
                   "made on line %u in file %s\n",
                   f_alloc_lines[i].line, f_alloc_lines[i].file);
          else
            printf("FORGE Mem debugger error: Memory Overrun of allocation "
                   "made on  line %u in file %s /* %s */\n",
                   f_alloc_lines[i].line, f_alloc_lines[i].file,
                   f_alloc_lines[i].allocs[j].comment);
          FORGE_CALL_ON_ERROR
          output = TRUE;
        }
        buf = ((unsigned char *)f_alloc_lines[i].allocs[j].buf) -
              FORGE_MEMORY_PRE_PADDIG;
        size = f_alloc_lines[i].allocs[j].size;
        ...
        if (k < FORGE_MEMORY_PRE_PADDIG) {
          if (f_alloc_lines[i].allocs[j].comment == NULL)
            printf("FORGE Mem debugger error: Memory underrun of allocation "
                   "made on line %u in file %s\n",
                   f_alloc_lines[i].line, f_alloc_lines[i].file);
          else
            printf("FORGE Mem debugger error: Memory underrun of allocation "
  ...
  for (i = 0; i < f_freed_memory_count && i < f_freed_memory_store; i++) {
    ...
    if (k < size) {
      if (f_freed_memory[i].realloc)
        printf("FORGE Mem debugger error: Pointer that was reallocated on line "
               "%u in file %s, and freed on line %u in file %s was written to "
               "%u bytes in to the allocation after being freed.\n",
               f_freed_memory[i].alloc_line, f_freed_memory[i].alloc_file,
               f_freed_memory[i].free_line, f_freed_memory[i].free_file,
               (unsigned int)k);
      else
        printf("FORGE Mem debugger error: Pointer that was allocated on line "
               "%u in file %s, and freed on line %u in file %s was written to "
               "%u bytes in to the allocation after being freed.\n",
               f_freed_memory[i].alloc_line, f_freed_memory[i].alloc_file,
               f_freed_memory[i].free_line, f_freed_memory[i].free_file,
...
void f_debug_mem_add(void *pointer, size_t size, char *file,
  ...
  if (i < f_alloc_line_count) {
    if (f_alloc_lines[i].alloc_alocated == f_alloc_lines[i].alloc_count) {
      ...
      f_alloc_lines[i].allocs =
          realloc(f_alloc_lines[i].allocs, (sizeof *f_alloc_lines[i].allocs) *
                                               f_alloc_lines[i].alloc_alocated);
      if (f_alloc_lines[i].allocs == NULL) {
        printf("FORGE Mem debugger error: Realloc returns NULL when trying to "
               "allocate %u bytes at line %u in file %s\n",
    ...
    f_alloc_lines[i].allocs[f_alloc_lines[i].alloc_count].active =
        forge_memory_active;
    ...
    if (i % 1024 == 0)
      f_alloc_lines =
          realloc(f_alloc_lines, (sizeof *f_alloc_lines) * (i + 1024));
...
void *f_debug_mem_malloc(size_t size, char *file, unsigned int line) {
  ...
  if (forge_memory_log_file != NULL && forge_memory_active)
    fprintf(forge_memory_log_file,
            "malloc %u bytes at pointer %p at %s line %u\n", (unsigned int)size,
            pointer, file, line);

  if (pointer == NULL) {
#ifdef FORGE_MEMORY_NULL_ALLOCATION_ERROR
    if (size > (size_t)(~((unsigned int)0)))
      printf("FORGE Mem debugger warning: malloc returns NULL at line %u in "
             "file %s\n",
             line, file);
    else
      printf("FORGE Mem debugger warning: malloc returns NULL when trying to "
             "allocate %u bytes at line %u in file %s\n",
             (unsigned int)size, line, file);
    if (f_alloc_mutex != NULL)
      f_alloc_mutex_unlock(f_alloc_mutex);
...
void *f_debug_mem_calloc(size_t num, size_t size, char *file,
                         unsigned int line) {
  unsigned char *pointer;
#ifdef FORGE_MEMORY_CHECK_ALWAYS
  f_debug_mem_check_bounds();
  ...
  if (forge_memory_log_file != NULL && forge_memory_active)
    fprintf(forge_memory_log_file,
            "Calloc %u bytes at pointer %p at %s line %u\n", (unsigned int)size,
            pointer, file, line);

  if (pointer == NULL) {
#ifdef FORGE_MEMORY_NULL_ALLOCATION_ERROR
    if (size > (size_t)(~((unsigned int)0)))
      printf("FORGE Mem debugger varning: Calloc returns NULL at line %u in "
             "file %s\n",
             line, file);
    else
      printf("FORGE Mem debugger varning: Calloc returns NULL when trying to "
             "allocate %u bytes at line %u in file %s\n",
             (unsigned int)size, line, file);
    if (f_alloc_mutex != NULL)
      f_alloc_mutex_unlock(f_alloc_mutex);
...
boolean f_debug_mem_remove(unsigned char *buf, char *file, unsigned int line,
                           boolean realloced, size_t *size) {
  ForgeMemFreeBuf *f;
  unsigned int i, j, k;
  size_t distance;

#if defined(FORGE_DOUBLE_FREE_CHECK) || defined(FORGE_USE_AFTER_FREE_CHECK)
  if (f_freed_memory_count % 1024 == 0 &&
      f_freed_memory_count < f_freed_memory_store) {
    if (f_freed_memory_count == 0)
      f_freed_memory = malloc((sizeof *f_freed_memory) * 1024);
    else
      f_freed_memory =
          realloc(f_freed_memory,
                  (sizeof *f_freed_memory) * (f_freed_memory_count + 1024));
  }
#ifdef FORGE_USE_AFTER_FREE_CHECK
  if (f_freed_memory_count >= f_freed_memory_store) {
    if (f_freed_memory[f_freed_memory_count % f_freed_memory_store].pointer !=
        NULL) {
      free(((unsigned char *)
                f_freed_memory[f_freed_memory_count % f_freed_memory_store]
                    .pointer) -
           FORGE_MEMORY_PRE_PADDIG);
      f_freed_memory[f_freed_memory_count % f_freed_memory_store].pointer =
          NULL;
    }
  }
#endif
  ...
  for (i = 0; i < f_alloc_line_count; i++) {
    for (j = 0; j < f_alloc_lines[i].alloc_count; j++) {
      if (f_alloc_lines[i].allocs[j].buf == buf) {
        ...
        if (k < FORGE_MEMORY_PRE_PADDIG) {
          unsigned int *a = NULL;
          printf("FORGE Mem debugger error: Buffer underrun of allocation made "
                 "on line %u in file %s\n",
                 f_alloc_lines[i].line, f_alloc_lines[i].file);
          FORGE_CALL_ON_ERROR;
        }
        for (k = 0; k < FORGE_MEMORY_OVER_ALLOC - FORGE_MEMORY_PRE_PADDIG; k++)
          if (((unsigned char *)buf)[f_alloc_lines[i].allocs[j].size +
                                     FORGE_MEMORY_PRE_PADDIG + k] !=
              F_MEMORY_MAGIC_NUMBER)
            break;
        if (k < FORGE_MEMORY_OVER_ALLOC - FORGE_MEMORY_PRE_PADDIG) {
          unsigned int *a = NULL;
          printf("FORGE Mem debugger error: Buffer overrun of allocation made "
                 "on line  %u in file %s\n",
                 f_alloc_lines[i].line, f_alloc_lines[i].file);
          FORGE_CALL_ON_ERROR;
        }
        memset(buf, F_MEMORY_FREED,
               f_alloc_lines[i].allocs[j].size + FORGE_MEMORY_OVER_ALLOC);
      ...
      if ((unsigned char *)f_alloc_lines[i].allocs[j].buf <
          ...
          (unsigned char *)f_alloc_lines[i].allocs[j].buf +
              ...
              (unsigned char *)buf) {
        printf("FORGE Mem debugger error: Trying to free pointer %p that is "
               "not at the start (%i bytes in) of allocation made on line %u "
               "in file %s\n",
               f_freed_memory[i].pointer,
  ...
  for (i = 0; i < f_freed_memory_count && i < f_freed_memory_store; i++) {
    if (f != &f_freed_memory[i] && buf == f_freed_memory[i].pointer) {
      unsigned int *a = NULL;
      if (f->realloc)
        printf("FORGE Mem debugger error: Pointer %p s freed twice! it was "
               "freed on line %u in %s, was reallocated (%u bytes) on line %u "
               "in file %s\n",
               f_freed_memory[i].pointer, f_freed_memory[i].free_line,
               f_freed_memory[i].free_file,
               (unsigned int)f_freed_memory[i].size,
               f_freed_memory[i].alloc_line, f_freed_memory[i].alloc_file);
      else
        printf("FORGE Mem debugger error: Pointer %p is freed twice! it was "
               "freed on line %u in %s, was allocated (%u bytes) on line %u in "
               "file %s\n",
               f_freed_memory[i].pointer, f_freed_memory[i].free_line,
               f_freed_memory[i].free_file,
               (unsigned int)f_freed_memory[i].size,
  ...
  if (forge_memory_stack_size != 0) {
    if (forge_memory_stack_pointer <= buf &&
        &forge_memory_stack_pointer[forge_memory_stack_size] > buf) {
      printf("FORGE Mem debugger error: Trying to free stack pointer on line "
             "%u in file %s.\n",
             line, file);
      FORGE_CALL_ON_ERROR;
      return TRUE;
    ...
    if (distance < FORGE_STACK_SIZE) {
      printf("FORGE Mem debugger error: Trying to free pointer, not allocated "
             "as far as FORGE knows on line %u in file %s. Very likly to be "
             "stack pointer (%u bytes from known stack pointer)\n",
  ...
  printf("FORGE Mem debugger varning: Trying to free pointer, not allocated as "
         "far as FORGE knows on line %u in file %s. \n",
         line, file);
  free(buf);
...
void f_debug_mem_free(void *buf, char *file, unsigned int line) {
  ...
  if (f_debug_mem_remove(buf, file, line, FALSE, &size))
    if (forge_memory_log_file != NULL && forge_memory_active)
      fprintf(forge_memory_log_file,
              "Free %u bytes at pointer %p at %s line %u\n", (unsigned int)size,
              buf, file, line);
...
void *f_debug_mem_realloc(void *pointer, size_t size, char *file,
  ...
  if (pointer == NULL) {
    ...
    printf("FORGE Mem debugger warning: Realocating NULL in %s line %u. UB "
           "since C23.\n",
           file, line);
#endif
  ...
  if (i == f_alloc_line_count) {
    printf("FORGE Mem debugger error: Trying to reallocate pointer %p in %s "
        ...
        for (k = 0; k < f_alloc_lines[i].allocs[j].size; k++) {
          if (&buf[k] == pointer) {
            printf("Trying to reallocate pointer %u bytes (out of %u) in to "
                   ...
                   (unsigned int)k,
                   (unsigned int)f_alloc_lines[i].allocs[j].size,
...
#ifdef FORGE_WARN_ON_REALLOC_NULL
    if (size == 0)
      printf("FORGE Mem debugger warning. Realocating pointer to zero size on "
             "line %u in file %s. UB since C23.\n",
             line, file);
#endif;
  ...
  if (pointer2 == NULL) {
    printf("FORGE Mem debugger warning: Realloc returns NULL when trying to "
           "allocate %u bytes at line %u in file %s\n",
           (unsigned int)size, line, file);
    FORGE_CALL_ON_ERROR;
    if (f_alloc_mutex != NULL)
  ...
  if (move > size) {
    ...
    memset(&pointer2[FORGE_MEMORY_PRE_PADDIG + move], F_MEMORY_MAGIC_NUMBER,
           FORGE_MEMORY_OVER_ALLOC - FORGE_MEMORY_PRE_PADDIG);
  } else {
    memcpy(&pointer2[FORGE_MEMORY_PRE_PADDIG], pointer, move);
    memset(&pointer2[FORGE_MEMORY_PRE_PADDIG + move], F_MEMORY_INITIALIZATION,
           size - move);
    memset(&pointer2[FORGE_MEMORY_PRE_PADDIG + size], F_MEMORY_MAGIC_NUMBER,
           FORGE_MEMORY_OVER_ALLOC - FORGE_MEMORY_PRE_PADDIG);
  }
  ...
  if (forge_memory_log_file != NULL && forge_memory_active)
    fprintf(forge_memory_log_file,
            "Relloc %u bytes at pointer %p to %u bytes at pointer %p at %s "
            "line %u\n",
            (unsigned int)size, pointer, (unsigned int)move, pointer2, file,
            line);
  if (f_alloc_mutex != NULL)
    f_alloc_mutex_unlock(f_alloc_mutex);
  return pointer2;
    ...
    if (min_allocs < f_alloc_lines[i].alocated - f_alloc_lines[i].freed) {
      ...
      if (alloc_count > 0) {
        ...
        printf(" - Bytes allocated: %u\n - Allocations: %u\n - Frees: %u\n\n",
               (unsigned int)f_alloc_lines[i].size, (unsigned int)alloc_count,
               (unsigned int)f_alloc_lines[i].freed);
        for (j = 0; j < f_alloc_lines[i].alloc_count; j++)
          if (f_alloc_lines[i].allocs[j].comment != NULL)
...
void *f_debug_mem_query_allocation(void *pointer, unsigned int *line,
  ...
  for (i = 0; i < f_alloc_line_count; i++) {
    for (j = 0; j < f_alloc_lines[i].alloc_count; j++) {
      if ((unsigned char *)f_alloc_lines[i].allocs[j].buf <=
          ...
          (unsigned char *)f_alloc_lines[i].allocs[j].buf +
                  f_alloc_lines[i].allocs[j].size >
              (unsigned char *)pointer) /* technically UB */
      {
        if (line != NULL)
...
boolean f_debug_mem_query_is_allocated(void *pointer, size_t size,
    ...
    for (j = 0; j < f_alloc_lines[i].alloc_count; j++) {
              ...
              (unsigned char *)pointer) /* technically UB */
      {
        if (((unsigned char *)f_alloc_lines[i].allocs[j].buf) +
          ...
          printf(
              "FORGE Mem debugger error: Not enough memory to access pointer "
              "%p, %u bytes missing\n",
              pointer,
              (unsigned int)(((unsigned char *)f_alloc_lines[i].allocs[j].buf) +
                             f_alloc_lines[i].allocs[j].size) -
  ...
  for (i = 0; i < f_freed_memory_count; i++) {
    if (pointer >= f_freed_memory[i].pointer &&
        ((unsigned char *)f_freed_memory[i].pointer) + f_freed_memory[i].size >=
            ((unsigned char *)pointer) + size) /* technically UB */
    {
      printf("FORGE Mem debugger error: Pointer %p was freed on line %u in "
             "file %s\n",
             pointer, f_freed_memory[i].free_line, f_freed_memory[i].free_file);
    }
  }
  if (forge_memory_stack_size != 0 && pointer >= forge_memory_stack_pointer &&
      &forge_memory_stack_pointer[forge_memory_stack_size] > pointer) {
    if (&forge_memory_stack_pointer[forge_memory_stack_size] <
        &((unsigned char *)pointer)[size])
      printf(
          "FORGE Mem debugger Eror: memmory is in stack, but does not fit.\n",
          pointer, f_freed_memory[i].free_line, f_freed_memory[i].free_file);
    else
  ...
  if (ignore_not_found)
    return FALSE;
  printf(
      "FORGE Mem debugger warning: No matching memory for pointer %p found!\n",
      pointer);
  return FALSE;
}
  ...
  for (i = 0; i < f_alloc_line_count; i++) {
    for (j = 0; j < f_alloc_lines[i].alloc_count; j++) {
      if (f_alloc_lines[i].allocs[j].active) {
        ...
        if (forge_memory_stack_size != 0) {
          for (k = 0; k < size; k++) {
            if (buf[k] >= forge_memory_stack_pointer &&
                buf[k] < &forge_memory_stack_pointer[forge_memory_stack_size]) {
              if (f_alloc_lines[i].allocs[j].comment == NULL)
                printf(
                    "FORGE Mem debugger varning: Suspected reference to stack "
                    "variable %u bytes in to in allocation made on line %u in "
                    "file %s (%u bytes off known stack pointer)\n",
                    (unsigned int)(k * sizeof(void *)), f_alloc_lines[i].line,
                    f_alloc_lines[i].file, (unsigned int)best);
              else
                printf(
                    "FORGE Mem debugger varning: Suspected reference to stack "
                    "variable %u bytes in to in allocation made on line %u in "
                    "file %s /* %s */ (%u bytes off known stack pointer)\n",
                    (unsigned int)(k * sizeof(void *)), f_alloc_lines[i].line,
                    f_alloc_lines[i].file, f_alloc_lines[i].allocs[j].comment,
                    (unsigned int)best);
            }
          }
        } else {
          ...
          if (found != ~0) {
            if (f_alloc_lines[i].allocs[j].comment == NULL)
              printf("FORGE Mem debugger varning: Suspected reference to stack "
                     "variable %u bytes in to in allocation made on line %u in "
                     "file %s (%u bytes off known stack pointer)\n",
                     (unsigned int)found, f_alloc_lines[i].line,
                     f_alloc_lines[i].file, (unsigned int)best);
            else
              printf("FORGE Mem debugger varning: Suspected reference to stack "
                     "variable %u bytes in to in allocation made on line %u in "
...
boolean f_debug_find_pointer_in_memory(void *pointer) {
  ...
  for (j = 0; j < forge_memory_stack_size / sizeof(void *) && buf[j] != pointer;
       j++)
    ;
  if (f_alloc_mutex != NULL)
    f_alloc_mutex_unlock(f_alloc_mutex);
    ...
    if (f_alloc_lines[i].alloc_count >= minimum_allocations) {
      ...
      if (found != f_alloc_lines[i].alloc_count) {
        if (forge_memory_stack_size)
          printf("FORGE Mem debugger Error: Cant find any reference in heap "
                 "memory or stack of %u out of %u allocations made on line %u "
                 "in file %s\n",
                 f_alloc_lines[i].alloc_count - (unsigned int)found,
                 f_alloc_lines[i].alloc_count, f_alloc_lines[i].line,
                 f_alloc_lines[i].file);
        else
          printf(
              "FORGE Mem debugger Warning: Cant find any reference in heap "
              "memory of %u out of %u allocations made on line %u in file %s\n",
              f_alloc_lines[i].alloc_count - (unsigned int)found,
              f_alloc_lines[i].alloc_count, f_alloc_lines[i].line,
              f_alloc_lines[i].file);
      }
...
#undef F_MEMORY_INTERNAL
#undef F_NO_MEMORY_DEBUG