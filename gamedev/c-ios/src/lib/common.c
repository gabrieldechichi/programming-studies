...
size_t f32_to_str(float32 n, char *str) {
  ...
  float32 fractional_part = n - (float32)whole_part;
  int32 sign = (n < 0) ? -1 : 1;
...
size_t double_to_str(double n, char *str) {
  ...
  if (n == 1.0/0.0) {  // Positive infinity
    *ptr++ = 'i'; *ptr++ = 'n'; *ptr++ = 'f';
    *ptr = '\0';
    return 3;
  }
  
  if (n == -1.0/0.0) {  // Negative infinity
    *ptr++ = '-'; *ptr++ = 'i'; *ptr++ = 'n'; *ptr++ = 'f';
    *ptr = '\0';
    return 4;
  }
  ...
  if (n == whole_part && n >= -9007199254740992.0 && n <= 9007199254740992.0) {
    ...
    if (frac_part > 0.0000001) {  // Only show decimals if meaningful
      *ptr++ = '.';
      ...
      // Remove trailing zeros, but keep at least one decimal if we started with any
...
size_t fmt_string(char *buffer, size_t buffer_size, const char *fmt,
  ...
  while (fmt[fmt_idx] != '\0' && buffer_idx < buffer_size - 1) {
    ...
    if (arg_idx < args->args_size) {
      ...
      switch (arg.type) {
      case FMTARG_FLOAT:
        arg_size = f32_to_str(arg.value_f32, &buffer[buffer_idx]);
        break;
      case FMTARG_INT:
        arg_size = i32_to_str(arg.value_i32, &buffer[buffer_idx]);
        break;
      case FMTARG_UINT:
        arg_size = u32_to_str(arg.value_u32, &buffer[buffer_idx]);
        break;
      case FMTARG_CHAR:
        arg_size = 1;
      ...
      case FMTARG_STR:
        ...
        break;
      case FMTARG_HEX:
        arg_size = hex32_to_str(arg.value_hex, &buffer[buffer_idx]);
        break;
      default:
        break;
      }
