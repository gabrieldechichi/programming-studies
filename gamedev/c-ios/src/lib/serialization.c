...
bool32 write_u8(BinaryWriter *writer, u8 *arr, u32 len) {
  u32 end_offset = writer->cur_offset + sizeof(u8) * len;
...
bool32 write_u32(BinaryWriter *writer, u32 v) {
  u32 end_offset = writer->cur_offset + sizeof(u32);
  bool32 within_range = end_offset <= writer->len;
  assert(within_range);
  if (!within_range) {
    return false;
...
bool32 write_i32(BinaryWriter *writer, i32 v) {
  u32 end_offset = writer->cur_offset + sizeof(i32);
  bool32 within_range = end_offset <= writer->len;
  assert(within_range);
  if (!within_range) {
    return false;
...
bool32 write_f32(BinaryWriter *writer, f32 v) {
  u32 end_offset = writer->cur_offset + sizeof(f32);
  bool32 within_range = end_offset <= writer->len;
  assert(within_range);
  if (!within_range) {
    return false;
...
bool32 write_u32_array(BinaryWriter *writer, u32 *arr, u32 len) {
  ...
  u32 end_offset = writer->cur_offset + sizeof(u32) * len;
  bool32 within_range = end_offset <= writer->len;
  assert(within_range);
  if (!within_range) {
    return false;
...
bool32 write_f32_array(BinaryWriter *writer, f32 *arr, u32 len) {
  u32 end_offset = writer->cur_offset + sizeof(f32) * len;
  bool32 within_range = end_offset <= writer->len;
  assert(within_range);
  if (!within_range) {
    return false;
...
bool32 read_u8_array(BinaryReader *reader, u8 *arr, u32 len) {
  u32 end_offset = reader->cur_offset + sizeof(u8) * len;
...
bool32 read_i32(BinaryReader *reader, i32 *v) {
  u32 end_offset = reader->cur_offset + sizeof(i32);
  bool32 within_range = end_offset <= reader->len;
  assert(within_range);
  if (!within_range) {
    return false;
...
bool32 read_f32(BinaryReader *reader, f32 *v) {
  u32 end_offset = reader->cur_offset + sizeof(f32);
  bool32 within_range = end_offset <= reader->len;
  assert(within_range);
  if (!within_range) {
    return false;
