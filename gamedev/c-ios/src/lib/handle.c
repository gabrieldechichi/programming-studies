...
HandleArray _ha_init(Allocator *allocator, u32 initial_capacity,
  ...
  array.handles = slice_new_ALLOC(allocator, Handle, initial_capacity);
  array.sparse_indexes =
      slice_new_ALLOC(allocator, SparseIndex, initial_capacity);
...
Handle _ha_add(HandleArray *array, void *item) {
  if (array->next >= array->sparse_indexes.len) {
    ...
    slice_append(array->sparse_indexes, sparse_idx);
    slice_append(array->handles, handle);
    array->next++;
    return handle;
  } else {
    // we can reuse a slot
...
void _ha_remove(HandleArray *array, Handle handle) {
  ...
  slice_remove_swap(array->handles, remove_idx);
