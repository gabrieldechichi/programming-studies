...
static ArenaAllocator arena_from_buffer(uint8 *buffer, size_t capacity) {
  return cast(ArenaAllocator){
      .buffer = buffer, .capacity = capacity, .offset = 0};
}

static b32 sub_arena_from_arena(ArenaAllocator *a, size_t capacity,
...
static void *arena_alloc_align(ArenaAllocator *a, size_t size, size_t align) {
  assert(a->buffer);
  assert(a->capacity);
  uintptr curr_ptr = (uintptr)a->buffer + (uintptr)a->offset;
  uintptr offset = align_forward(curr_ptr, align);
...
static void *arena_alloc(ArenaAllocator *a, size_t size) {
  return arena_alloc_align(a, size, DEFAULT_ALIGNMENT);
}

static void *arena_realloc(ArenaAllocator *a, void *ptr, size_t size) {
  if (!ptr) {
...
static void arena_reset(ArenaAllocator *arena) { arena->offset = 0; }

static void arena_destroy(ArenaAllocator *arena) {
  arena->buffer = NULL;
  arena->capacity = 0;
  arena->offset = 0;
}
...
static void *arena_alloc_impl(void *ctx, size_t size, size_t align) {
  return arena_alloc_align((ArenaAllocator *)ctx, size, align);
}

static void *arena_realloc_impl(void *ctx, void *ptr, size_t size) {
  return arena_realloc((ArenaAllocator *)ctx, ptr, size);
}

static void arena_reset_impl(void *ctx) { arena_reset((ArenaAllocator *)ctx); }
static size_t arena_capacity_impl(void *ctx) {
  return ((ArenaAllocator *)ctx)->capacity;
}
static size_t arena_commited_size_impl(void *ctx) {
  return arena_committed_size((ArenaAllocator *)ctx);
}
static size_t arena_free_size_impl(void *ctx) {
  return arena_free_size((ArenaAllocator *)ctx);
}

static void arena_destroy_impl(void *ctx) { arena_destroy((ArenaAllocator *)ctx); }

static Allocator make_arena_allocator(ArenaAllocator *arena) {
