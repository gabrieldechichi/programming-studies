...
slice_define(SharedAnimation);

typedef struct {
  u32 shared_animation_idx;
  mat4 model_matrix;
} AnimatedEntity;
...
typedef struct {
  ...
  SharedAnimation_Slice shared_animations;
  SharedAnimationEntity_Slice animated_entities;
} GameState;
...
void spawn_more_animated_meshes() {
  ...
  for (u32 j = 0; j < total_count; j++) {
    AnimatedEntity *animated_model =
        &game_state->animated_entities.items[j];
    u32 grid_x = j % grid_size;
    u32 grid_z = j / grid_size;

    animated_model->shared_animation_idx = 0;
...
void gym_init(GameMemory *memory) {
  ...
  assert(game_state);
  glm_vec3(cast(vec3){-0.5, 20, 40}, game_state->camera.pos);
  ...
  game_state->shared_animations =
      slice_new_ALLOC(&ctx->allocator, SharedAnimation, 10);

  game_state->animated_entities = slice_new_ALLOC(
      &ctx->allocator, AnimatedEntity, MAX_ANIM_INSTANCES);

  game_state->temp_anim_count = 1500;
...
void gym_update_and_render(GameMemory *memory) {
  ...
  {
    if (load_sphere_op < 0) {
      // load_sphere_op = platform_start_read_file("gorilla.hmobj");
      load_sphere_op = platform_start_read_file("animation_test.hmobj");
    }
    if (load_tex_op < 0) {
      // load_tex_op = platform_start_read_file("Gorilla_BaseMap.png");
      load_tex_op = platform_start_read_file("white_pixel.png");
    }
    if (load_tex_op_2 < 0) {
      // load_tex_op_2 = platform_start_read_file("Short_Hair.png");
      load_tex_op_2 = platform_start_read_file("white_pixel.png");
    }
    if (load_anim_op < 0) {
      // load_anim_op = platform_start_read_file("gorilla_anim.hasset");
    ...
    if (sphere_mesh && tex_data && tex_data_2 && test_anim_asset) {
      ...
      for (u32 i = 0; i < sphere_mesh->num_meshes; i++) {
        ...
        success &= renderer_create_skmaterial(
            texture_handles.items[i], i == 0 ? mat_color_1 : mat_color_2,
            &material_handles.items[i]);
        assert(success);
      }
      tex_data = NULL;
      tex_data_2 = NULL;
    ...
    if (sphere_mesh != NULL && mesh_handles.len > 0 &&
      ...
      SharedAnimation shared_animation = {0};
      shared_animation.skinned_model.material_handles = material_handles;

      shared_animation.skinned_model.joint_matrices =
          arr_new_ALLOC(&ctx->allocator, mat4, sphere_mesh->len_joints);

      shared_animation.skinned_model.mesh_handles = mesh_handles;

      shared_animation.animation_state.animation = anim;
      shared_animation.animation_state.speed = 1.2f;
      shared_animation.animation_state.weight = 1.0f;
      shared_animation.animation_state.time = 1.2;

      slice_append(game_state->shared_animations, shared_animation);

      sphere_mesh = NULL;
    }

    if (game_state->shared_animations.len > 0) {
  ...
  // animation
  arr_foreach_ptr(game_state->shared_animations, shared_anim) {
    animation_update(&shared_anim->animation_state, dt);
    animation_evaluate(&shared_anim->animation_state,
                       shared_anim->skinned_model.joint_matrices);
  }
  ...
  // render models
  {
    arr_foreach_ptr(game_state->animated_entities, 
      ...
      SharedAnimation *shared_anim =
          &game_state->shared_animations
               .items[animated_entity->shared_animation_idx];

      renderer_draw_skmeshes(shared_anim->skinned_model.mesh_handles.items,
                             shared_anim->skinned_model.material_handles.items,
                             shared_anim->skinned_model.mesh_handles.len,
                             animated_entity->model_matrix,
                             shared_anim->skinned_model.joint_matrices.items,
                             shared_anim->skinned_model.joint_matrices.len);
    }
