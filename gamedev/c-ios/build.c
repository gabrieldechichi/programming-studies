...
#define MACOS_VENDOR_COMPILE_FLAGS                                             \
  "-x objective-c -Isrc -Isrc/vendor -DMACOS=1 " VENDOR_RELAXED_FLAGS
#define MACOS_MAIN_COMPILE_FLAGS                                               \
  "-x objective-c -Isrc -Isrc/vendor -DMACOS=1 " MAIN_STRICT_FLAGS
#define MACOS_LINK_FLAGS                                                       \
  "-x objective-c -Isrc -Isrc/vendor -DMACOS=1 " MAIN_STRICT_FLAGS " -v"
...
#define IOS_VENDOR_COMPILE_FLAGS                                               \
  ...
  "-DIOS=1 " VENDOR_RELAXED_FLAGS
#define IOS_MAIN_COMPILE_FLAGS                                                 \
  "-x objective-c -miphoneos-version-min=12.0 -Isrc -Isrc/vendor "             \
  "-DIOS=1 " MAIN_STRICT_FLAGS
#define IOS_LINK_FLAGS                                                         \
  "-x objective-c -arch arm64 -Isrc -Isrc/vendor -DIOS=1 " MAIN_STRICT_FLAGS
...
#define WINDOWS_VENDOR_COMPILE_FLAGS                                           \
  "-Isrc -Isrc/vendor -target " WINDOWS_TARGET                                 \
  " -DWIN64=1 " VENDOR_RELAXED_FLAGS
#define WINDOWS_MAIN_COMPILE_FLAGS                                             \
  "-Isrc -Isrc/vendor -target " WINDOWS_TARGET " -DWIN64=1 " MAIN_STRICT_FLAGS
// -Wl,--subsystem,windows prevents console window from appearing
#define WINDOWS_LINK_FLAGS                                                     \
  "-target " WINDOWS_TARGET " -DWIN64=1 -Wl,--subsystem,windows"
// Windows libraries needed by Sokol
#define WINDOWS_LIBS                                                           \
  "-ld3d11 -ldxgi -lgdi32 -lole32 -lkernel32 -luser32 -lshell32"
...
int build_macos(const char *build_type) {
  ...
  if (need_vendor) {
    ...
    char cmd[1024];
    snprintf(cmd, sizeof(cmd), "%s %s %s -c %s -o %s", CC,
             MACOS_VENDOR_COMPILE_FLAGS, vendor_build_flags, vendor_src,
             vendor_obj);
...
int build_ios(const char *build_type) {
  ...
  if (need_vendor) {
    ...
    char cmd[1024];
    snprintf(cmd, sizeof(cmd), "%s %s %s %s -arch arm64 -c %s -o %s", IOS_SDK,
             CC, IOS_VENDOR_COMPILE_FLAGS, vendor_build_flags, vendor_src,
             vendor_obj);
  ...
  if (need_main) {
    ...
    snprintf(cmd, sizeof(cmd), "%s %s %s %s %s %s %s -o %s %s", IOS_SDK, CC,
             IOS_MAIN_COMPILE_FLAGS, build_flags, main_src, LINK_RESET_FLAGS,
             vendor_obj, app_target, IOS_FRAMEWORKS);
...
int build_windows(const char *build_type) {
  ...
  if (system("which zig > /dev/null 2>&1") != 0) {
    fprintf(
        stderr,
        "❌ zig not found! Install it from https://ziglang.org/download/\n");
    return 1;
  }
  ...
  if (need_vendor) {
    ...
    char cmd[1024];
    snprintf(cmd, sizeof(cmd), "%s %s %s -c %s -o %s", ZIG_CC,
             WINDOWS_VENDOR_COMPILE_FLAGS, vendor_build_flags, vendor_src,
             vendor_obj);
  ...
  if (need_main) {
    ...
    snprintf(cmd, sizeof(cmd), "%s %s -Isrc -Isrc/vendor %s %s %s %s -o %s %s",
             ZIG_CC, WINDOWS_LINK_FLAGS, MAIN_STRICT_FLAGS, build_flags,
             main_src, vendor_obj, app_target, WINDOWS_LIBS);
...
int compile_shaders(const char *target_platform) {
  ...
  while (fgets(shader_path, sizeof(shader_path), find_fp)) {
    ...
    snprintf(output_path, sizeof(output_path), "src/shaders/%s_%s.h", basename,
             target_platform);

    // Check if shader needs recompilation
    if (file_exists(output_path) &&
        file_mtime(shader_path) <= file_mtime(output_path)) {
    ...
    snprintf(compile_cmd, sizeof(compile_cmd),
             "%s --input %s --output %s --slang %s", shdc_path, shader_path,
             output_path, slang);
    ...
    snprintf(generic_header_path, sizeof(generic_header_path),
             "src/shaders/%s.h", basename);
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    if (strcmp(argv[1], "ios") == 0) {
      ...
      fprintf(
          stderr,
          "Usage: %s [macos|ios|windows|ios-deploy|shaders] [debug|release]\n",
          argv[0]);
